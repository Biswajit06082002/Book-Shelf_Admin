...
const Products = (props) => {
  ...
  const deleteProduct = (product, idx) => {
    ...
    fetch('https://calm-teal-cockroach-hose.cyclic.app/deleteProduct', {
      method: 'DELETE',
      body: JSON.stringify({
          _id: product._id, 
      }),
  ...
  useEffect(() => {
    fetch('https://calm-teal-cockroach-hose.cyclic.app/getCategoryProducts', {
      method: 'POST',
      body: JSON.stringify({
      ...
      <div className="col-lg-3 col-md-6 col-sm-12 pb-1" key={product._id}>
        <div className="card product-item border-0 mb-4">
          <div className="card-header product-img position-relative overflow-hidden bg-transparent border p-0" style={{ height: '250px' }}>
            <img className="img-fluid w-100" src={`https://calm-teal-cockroach-hose.cyclic.app/${product.imageUrl1}`} alt="" />
          </div>
